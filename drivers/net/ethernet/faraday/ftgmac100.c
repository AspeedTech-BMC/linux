...
static bool ftgmac100_rx_packet(struct ftgmac100 *priv, int *processed)
{
	...
	map = le32_to_cpu(rxdes->rxdes3) |
	      (le32_to_cpu(rxdes->rxdes2 & FTGMAC100_RXDES2_RXBUF_BADR_HI)
	       << 16);
...
{
	dma_addr_t map =
		le32_to_cpu(txdes->txdes3) |
		(le32_to_cpu(txdes->txdes2 & FTGMAC100_TXDES2_TXBUF_BADR_HI)
		 << 16);
	size_t len;
					     ...
					     struct net_device *netdev)
{
	...
	for (i = 0; i < nfrags; i++) {
		...
		txdes->txdes1 = 0;
		txdes->txdes2 =
			cpu_to_le32(FIELD_PREP(FTGMAC100_TXDES2_TXBUF_BADR_HI,
					       upper_32_bits((ulong)map)));
		txdes->txdes3 = cpu_to_le32(lower_32_bits(map));
...
static void ftgmac100_init_rings(struct ftgmac100 *priv)
{
	...
	for (i = 0; i < priv->rx_q_entries; i++) {
		...
		rxdes->rxdes0 = 0;
		rxdes->rxdes2 =
			cpu_to_le32(FIELD_PREP(FTGMAC100_RXDES2_RXBUF_BADR_HI,
					       upper_32_bits(priv->rx_scratch_dma)));
		rxdes->rxdes3 =
			cpu_to_le32(lower_32_bits(priv->rx_scratch_dma));
	}
